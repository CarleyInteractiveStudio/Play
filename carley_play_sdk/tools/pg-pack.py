...
def pack_pg(title, developer, bin_path, icon_path, output_path):
    ...
    # uint32_t code_size, uint32_t icon_offset, uint32_t icon_size, uint32_t entry_point, uint32_t checksum

    fmt = "2s B B 64s 64s I I I I I"
    sig = b'PG'
    ver = 1
    pad = 0
    ...
    header = struct.pack(fmt, sig, ver, pad, title_b, dev_b, code_size, icon_offset, icon_size, entry_point, checksum)
